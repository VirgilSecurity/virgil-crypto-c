...
{
    memset( ctx, 0, sizeof( ed25519_sha512_context ) );
}
...
void ed25519_sha512_free( ed25519_sha512_context *ctx )
{
    ...
    mbedtls_zeroize( ctx, sizeof( ed25519_sha512_context ) );
}
                    ...
                    size_t ilen )
{
    ...
    if( left && ilen >= fill )
    {
        ...
        ed25519_sha512_process( ctx, ctx->buffer );
        input += fill;
        ilen  -= fill;
        left = 0;
    }
    ...
    while( ilen >= 128 )
    {
        ed25519_sha512_process( ctx, input );
        input += 128;
        ilen  -= 128;
    }
...
void ed25519_sha512_finish( ed25519_sha512_context *ctx, unsigned char output[64] )
{
    ...
    ed25519_sha512_update( ctx, sha512_padding, padn );
    ed25519_sha512_update( ctx, msglen, 16 );
...
{
    ed25519_sha512_context ctx;

    ed25519_sha512_init( &ctx );
    ed25519_sha512_starts( &ctx, is384 );
    ed25519_sha512_update( &ctx, input, ilen );
    ed25519_sha512_finish( &ctx, output );
    ed25519_sha512_free( &ctx );
}
