 ...
 * Modifications from upstream (see thirdparty/sss/VERSION and hazmat.h):
 *   - `#include "randombytes.h"` removed; the random polynomial coefficients
 *     are now supplied by the caller via the `random_data` parameter.
 *   - Public functions prefixed `vscf_sss_*`.
 * The GF(256) arithmetic below is byte-identical to upstream.
 * ---------------------------------------------------------------------------
 */
 ...
 * structs.
 *
 * `random_data` supplies `32 * (k - 1)` bytes of uniform randomness for the
 * higher-order polynomial coefficients (see hazmat.h). Upstream called
 * `randombytes()` here; virgil-crypto-c provides the entropy externally.
 */
void
vscf_sss_create_keyshares(sss_Keyshare *out, const uint8_t key[32], uint8_t n, uint8_t k, const uint8_t *random_data) {
