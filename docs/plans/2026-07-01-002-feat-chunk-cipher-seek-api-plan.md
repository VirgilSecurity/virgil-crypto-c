...
- **R5** — Nonce-safety and thread-safety warnings are authored in the **IR model doc-comments** (`class_chunk_cipher.xml`, Unit 1) so they propagate to all wrappers: per-call index uniqueness is the caller's responsibility (no enforcement — see Risks); a single instance is not concurrency-safe (one instance per thread).
...
- No locking / internal concurrency — thread-safety is achieved by the caller using one instance per thread (documented, not enforced).
...
- **`chunk_size` is required uniformly (`> 0`), not conditionally.** `encrypt_chunk` bakes `self->chunk_size` into the index-0 AAD, and the seek path does **not** inherit `start_decryption`'s default-to-65536 fallback. Requiring `chunk_size` for every `encrypt_at`/`decrypt_at` call (not just index-0/is_last) is simpler to document/test and eliminates a silent-AAD-mismatch trap where `chunk_size==0` produces a frame no conforming decryptor can authenticate (surfacing as `ERROR_AUTH_FAILED`, indistinguishable from tampering). This resolves the deferred question.
...
- **Thread-safety = one instance per thread.** `encrypt_at`/`decrypt_at` mutate the shared inner `aes256_gcm` (set_key/set_nonce/set_auth_data), so a single `vscf_chunk_cipher` instance is not concurrency-safe. Parallelism uses one instance per thread, each configured with the same key + initial_nonce (+ chunk_size). Documented in doc-comments; no locking added.
...
- **Author the R5 doc-comments here** (the `<method>` description text in the XML is what propagates to all wrappers): nonce-safety (each index encrypted at most once per `(key, initial_nonce)` — no enforcement), thread-safety (one instance per thread), whole-file-only framing (caller must know total chunk count; exactly one `is_last`; empty trailing FIN on exact multiples), and preconditions (key + nonce + chunk_size, `INITIAL` state).
...
| Caller shares one instance across threads → data race on inner GCM state | Med | High | Documented: one instance per thread, same key+nonce+chunk_size; no locking added |
...
- Doc-comments on `encrypt_at`/`decrypt_at` (authored in the XML, Unit 1) must state: (1) nonce-safety — each index encrypted at most once per `(key, initial_nonce)`, **not enforced**, caller's contract; (2) thread-safety — one instance per thread; (3) whole-file-only framing — caller must know the total chunk count; exactly one `is_last` at index `floor(N/C)`, empty on exact multiples; index 0 carries chunk_size; (4) preconditions — key + nonce + `chunk_size>0` + `INITIAL` state (no default-chunk_size fallback on the seek path).
