...
1. **Only a minimal validation helper exists today.** `tools/codegen/verify_foundation_validation_gate.sh` now gives us a scripted configure/build/test gate for `foundation`, and `tools/codegen/verify_foundation_enum_slice.sh` adds an out-tree smoke check for the enum pilot, but there is still no `foundation` equivalent of `tools/codegen/build_common_with_new_codegen.sh` with generate-apply-build-restore behavior. The first broader emitter follow-up should add that before migration expands beyond a tiny pilot.
2. **The shared-framework tests are still slice-focused rather than broad parity coverage.** `CG-017` and `CG-019` now cover `project_foundation.xml` loading, IR/output-target lowering, shared bootstrap routing, and the enum pilot surfaces, but wider parity fixtures will still be needed before broad emitter ownership.
...
4. **The documented narrow workflow is pilot-specific.** `common_bootstrap.py --project foundation` plus `verify_foundation_enum_slice.sh` now provide a repeatable workflow for the enum slice, but follow-up tasks will need a more general entity-selection/apply flow before coverage expands beyond this pilot.
...
- classes/modules beyond the enum slice, including `error`, `base64`, `oid`, `pem title`, `group session typedefs`, and the deeper key/implementor families
- generated build metadata such as `library/foundation/features.cmake` and `library/foundation/sources.cmake`
- a full `generate -> apply -> build -> restore` helper equivalent to `tools/codegen/build_common_with_new_codegen.sh`
- a preservation-focused diff harness that proves writes stay inside generated blocks for the broader `foundation` surface
- wrapper/downstream ownership; wrappers remain validation consumers rather than generator targets in this phase
