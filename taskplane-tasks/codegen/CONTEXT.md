...
The new codegen generates **567 files** for the foundation project (289 `.c` + 278 `.h`, including 56 `_internal.h`). Zero unexpected module skips. The generated output is applied temporarily by build/verify scripts and **never committed** to the branch.
...
| Metric | Count |
|--------|-------|
| Files generated by new codegen | 567 |
| Files only in legacy (not generated) | 139 |
...
|----------|-------|------------------------------|-------|
| **`_internal.h`** (internal headers) | 58 | ✅ Yes | **CG-054: Now generated** — 56 `_internal.h` rendered (53 impls + 3 classes with internal methods). **CG-055: Confirmed** 5 remaining legacy files are dead code (not in `sources.cmake`): `ec_alg_info` and `pkcs8_der_serializer` (deprecated aliases), `ecies` and `padding_cipher` (orphaned class internals), `recipient_cipher` (unreferenced). 56/56 matches build system. |
| **`_defs.h`** (struct definition headers, in `private/`) | 37 | ✅ Yes | Private struct layout definitions (e.g. `vscf_sha256_defs.h`). **CG-053: Now generated** — 75/78 _defs.h rendered (3 blocked by CG-052 type resolution). |
...
- **~187 legacy-only files not generated**: See [Files Not Generated](#files-not-generated-195-legacy-only) for full categorization. Major categories: `_internal.h` (58 — 56 now generated by CG-054, 5 confirmed dead by CG-055), `_internal.c` (4 — confirmed dead by CG-055), `_defs.h/.c` (73 — now generated by CG-053, 161 files covered), build system (6). Interface dispatch `.c` files (8) now generated by CG-051. Remaining gap is primarily build system files and deprecated aliases.
