...
- **`vscf_self_t` in constructor declarations**: CG-045 emits `vscf_self_t` for `class="self"` constructor arguments in public headers, but this type is only defined internally. 4 foundation build errors. (fix: CG-046)
- **Known module skips**: `c_module_vscf_key.xml` and `c_module_vscf_key_api.xml` skip with 'enum not found in IR: impl/tag'. The `impl/tag` enum is a generated enum in the impl module, not a standalone IR entity. (discovered during CG-044, fix: CG-047)
