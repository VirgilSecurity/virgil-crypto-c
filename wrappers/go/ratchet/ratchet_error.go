...
const (
    ...
    */
    RatchetErrorErrorKemFailed int = -30
    /*
    * Signing operation failed.
    */
    RatchetErrorErrorSignFailed int = -31
    /*
...
func RatchetErrorHandleStatus(status C.vscr_status_t) error {
    if status != C.vscr_status_SUCCESS {
        ...
        case C.vscr_status_ERROR_KEM_FAILED:
            return &RatchetError {int(status), "KEM encapsulate or decapsulate operation failed."}
        case C.vscr_status_ERROR_SIGN_FAILED:
            return &RatchetError {int(status), "Signing operation failed."}
