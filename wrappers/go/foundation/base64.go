...
func Base64EncodedLen(dataLen uint) uint {
    proxyResult := C.vscf_base64_encoded_len((C.size_t)(dataLen))
...
func Base64Encode(data []byte) []byte {
    strBuf, strBufErr := newBuffer(int(Base64EncodedLen(uint(len(data)))))
    if strBufErr != nil {
        return nil
    }
    defer strBuf.delete()
...
func Base64DecodedLen(strLen uint) uint {
    proxyResult := C.vscf_base64_decoded_len((C.size_t)(strLen))
...
func Base64Decode(str []byte) ([]byte, error) {
    dataBuf, dataBufErr := newBuffer(int(Base64DecodedLen(uint(len(str)))))
    if dataBufErr != nil {
    ...
    proxyResult := C.vscf_base64_decode(strData, dataBuf.ctx)
